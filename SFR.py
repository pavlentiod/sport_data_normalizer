...
def soup_main_info(soup: BS):  # Возвращает заголовки и код таблиц
    h2: list = [i.get_text(' ').upper() for i in soup.find_all('h2')]
    tables: ResultSet = soup.find_all('table')
    return h2, tables


def check_void_groups(soup: BS):  # Сколько людей в каждой группе
    h2, tables = soup_main_info(soup)
    number_of_sportsmens: dict = {h2[i]: len(bs(tables[i]).find_all('tr')) - 1 for i in
                                  rl(h2)}
    return number_of_sportsmens


""" РАБОТА С ОДНОЙ ГРУППОЙ """


def BS_group_data(group: str, soup: BS):  # Возвращает суп с таблицей(можно отправлять в обработчики имен и сплитов)
    # print('bs1')
    h2, tables = soup_main_info(soup)
...
def group_names(group: str, tr: ResultSet):  # убрана обрезка группы названия .split(' ')[0]
    tr_pt: BS = bs(tr[1])
    ...
    if " " in td_pr[0].get_text(' ') or "\n" in td_pr[0].get_text(' '):
        ...
        raise ValueError('Имена не найдены')
    names = [re.sub(' +', ' ', i) for i in names]
    names = [i if names.count(i) == 1 else f'{i.split("^")[0]}*{random.randint(0, 100)}^{group.upper()}' for i in names]
    return names


""" Проверка протоколов на наличие кп """


def group_results(tr: ResultSet, names: list):  # Возвращает словарь имя - результат
...
def points(tr: ResultSet, th: ResultSet, tm_index: int,
           names: list):  # Проверка на наличие рассева, возврат пунктов для каждого человека
    r = rl(tr)[:-1]
...
def group_frame(group: str, soup: BS):
    ...
    # обработка времен и создание словаря для каждого человека
    GD = {}
    for name in names:
        try:
            # print(name, splits_l[name])
            stages.append(f'group_frame name {name}')
            sportsmen_info = routes_and_splits_dict(points_l[name], splits_l[name])
        except ValueError:
            stages.append(f'group_frame error with {name}')
            sportsmen_info = {}
        sportsmen_info.setdefault('RES', results[name])
        stages.append('group_frame 9')
        GD.setdefault(name, sportsmen_info)
        stages.append('group_frame 10')
...
def SFR_EVENT(link: str):
    ...
    for group in groups:
        ...
        try:
            ...
            # print('ERROR WITH EVENT FILE WRITING', link, stages)
            return pd.DataFrame(), {}, log
